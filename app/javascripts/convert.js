...
(function() {
  var $, async, child_process, compress, convert_image, convert_to_tiff, electron, fs, kdu_command, opj_command, pather, prettysize, settings, tempfile, tiff2rgba, update_completed_number;
  ...
  compress = function(tif_tmp_rgba, output_file, async_callback) {
    var cmd, command_function;
    command_function = settings.get('jp2_binary') === 'opj' ? opj_command : kdu_command;
  ...
  convert_image = function(file_row, async_callback) {
    ...
    return async.series([
      function(callback) {
        ...
        return compress(tif_tmp_rgba, jp2_file, callback);
      }, function(callback) {
        var jp2_filesize;
  ...
  opj_command = function(tif, output_file) {
    return "opj_compress -i " + tif + " -o " + output_file + " -r 2.5 -n 7 -c \"[256,256]\"  -b \"64,64\" -p RPCL -SOP -t 1024,1024 -TP R";
  };
